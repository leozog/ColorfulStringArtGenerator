    ...
    , end_wrap{ end_wrap }
{
    ...
    const Vec2<double> start_nail_pos{ nail_positions[start_nail_id] };
    const Vec2<double> end_nail_pos{ nail_positions[end_nail_id] };

    const double nails_dist{ start_nail_pos.dist(end_nail_pos) };
    const Vec2<double> nails_dir{ (end_nail_pos - start_nail_pos).norm() };

    const double nail_string_dist{ nail_radius + string_radius };

    const int start_wrap_sign{ start_wrap == Wrap::CLOKWISE ? -1 : 1 };
    const int end_wrap_sign{ end_wrap == Wrap::CLOKWISE ? 1 : -1 };

    if (start_wrap == end_wrap) {
        const Vec2<double> nails_dir_perp{ nails_dir.perp() };
        ...
        const double alpha{ std::acos(nail_string_dist / (nails_dist / 2)) };
        const Vec2<double> start_string_dir_perp{ nails_dir.rotate(alpha * start_wrap_sign) };
        const Vec2<double> end_string_dir_perp{ nails_dir.rotate(alpha * end_wrap_sign) };
